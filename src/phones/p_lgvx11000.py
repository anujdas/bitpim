...
class pbfileentry(BaseProtogenClass):
    __fields=['entry_tag', 'unk4', 'mod_date', 'unk0', 'entry_number1', 'entry_number0', 'name', 'groups', 'emails', 'ringtone', 'wallpaper', 'numbertypes', 'unk2', 'numberindices', 'addressindex', 'unk3', 'memo', 'exit_tag', 'dontcare']
    ...
    def writetobuffer(self,buf,autolog=True,logtitle="<written data>"):
        ...
        if self.entry_tag==PB_ENTRY_SOR:
            ...
            try: self.__field_groups
                ...
                self.__field_groups=LIST(**{'elementclass': _gen_p_lgvx11000_103,  'length': NUMGROUPS })
    ...
    def __setfield_groups(self, value):
        if isinstance(value,LIST):
            ...
            self.__field_groups=LIST(value,**{'elementclass': _gen_p_lgvx11000_103,  'length': NUMGROUPS })
...
class _gen_p_lgvx11000_103(BaseProtogenClass):
    ...
    def __init__(self, *args, **kwargs):
        ...
        dict.update(kwargs)
        # Parent constructor
        super(_gen_p_lgvx11000_103,self).__init__(**dict)
        if self.__class__ is _gen_p_lgvx11000_103:
            self._update(args,dict)
    ...
    def _update(self, args, kwargs):
        super(_gen_p_lgvx11000_103,self)._update(args,kwargs)
        keys=kwargs.keys()
        for key in keys:
            if key in self.__fields:
                setattr(self, key, kwargs[key])
        ...
        # Were any unrecognized kwargs passed in?
        if __debug__:
            self._complainaboutunusedargs(_gen_p_lgvx11000_103,kwargs)
        if len(args):
